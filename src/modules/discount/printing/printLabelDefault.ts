...
// Fungsi pure yang menyiapkan data label dari satu hasil scan
export const buildPrintLabelPayload = (scan: any) => {
...
// Fungsi pure untuk membangun string ESC/POS berdasarkan payload label
export const buildPrintLabelEscPos = (payload: {
	name: string;
	...
	const { name, barcodeBaru, discountPercent, hargaAwal, hargaDiskon, qty, uom } = payload;
	let printData = '';
...
export const printScanLabel = async (scan: any) => {
	...
	try {
		...
		const payload = buildPrintLabelPayload(scan);
		const printData = buildPrintLabelEscPos(payload);
